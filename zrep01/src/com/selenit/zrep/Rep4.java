...
public class Rep4 {
	public static String build(List<String> aBody1, List<String> aBody2, String aParam) {
		...
		try {
		    ...
		    String enterpriseINN = x.trim().length() == 0? "-1": x;
		    ...
		    for (int i = 0; i < count; i++) {
		    	if (Report.legal(managerList, i, "contact").trim().length() > 0) {
		    ...
		    if (Report.legal(dbObject, "data.credit.who_init.direct").trim().length() > 0) { // ПРЯМОЕ ОБРАЩЕНИЕ ИНИЦИАТОРА
		    	...
		    	Report.zReplaceAll(aBody2, "1111111111", "-1");
		    }
            // (4)Информация о проекте, отрасль проекта
		    Report.zReplaceAll(aBody1, "ПРОЕКТ", Report.exam(dbObject, "data.info.project_name"));
            // (6) Цель кредита
		    Report.zReplaceAll(aBody1, "ЦЕЛЬ", Report.exam(dbObject, "data.info.project_name"));
            // (7) Планируемая сумма проекта млн. руб.
		    x = Report.legal(dbObject, "data.sum.sum_total");
		    String sum_total = x.trim().length() == 0? "-1": Integer.toString((int)(Float.parseFloat(x) / 1000));
		    Report.zReplaceAll(aBody2, "777", sum_total);
            // (8) Планируемая сумма кредита, млн. руб.
		    x = Report.legal(dbObject, "data.credit.sum");
		    String credit_sum = x.trim().length() == 0? "-1": Integer.toString((int)(Float.parseFloat(x) / 1000));
		    ...
		    String garant_sum = x.trim().length() == 0? "-1": Integer.toString((int)(Float.parseFloat(x) / 1000));
